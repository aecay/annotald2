module TreeEdit.Tree.View exposing
    ( labelString
    , terminalString
    , toPenn
    )
...
labelString tree =
    let
        index =
            .get Tree.index tree |> Maybe.map Index.string |> Maybe.withDefault ""

        label =
            .get Tree.label tree
    in
    if isTrace tree then
        label
    ...
    let
        nt info kids =
            let
                ...
                childString_ =
                    case Dict.get "ID" <| .metadata info of
                        Just id ->
                            childString ++ "\n(ID " ++ id ++ ")"
        ...
        t terminal =
            case terminal of
                ...
                -- TODO: encode comment for cs format
                _ ->
                    "(" ++ labelString tree ++ " " ++ terminalString tree ++ ")"
    in
    Tree.either t nt tree
